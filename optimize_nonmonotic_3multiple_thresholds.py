...
def calculate_cost(x, costs):
    ...
    if int(x['prediction']) == 1:
        ...
        if x['prefix_nr'] < x['case_length']:
            x['prefix_nr'] = x['prefix_nr'] + 1
            if optimalCosts > calculate_cost(x,costs):
                optimalCosts = calculate_cost(x,costs)
    else:
...
def evaluate_model_cost(args):
    ...
    costs = np.matrix([[lambda x: 0,
                        lambda x: c_miss],
                       [lambda x: c_action * (x['case_length'] - min(x['prefix_nr'], nonmonotonic_threshold) / x[
                           'case_length']) + (c_com * (x['case_length'] - x['prefix_nr'] / x['case_length'])),
                        lambda x: c_action * (x['case_length'] - min(x['prefix_nr'], nonmonotonic_threshold) / x[
                            'case_length']) + max((
                       ...
                       [lambda x: c_action * (x['case_length'] - min(x['prefix_nr'], nonmonotonic_threshold) / x[
                           'case_length']) * 3 + (c_com * (x['case_length'] - x['prefix_nr'] / x['case_length'])) / 2,
                        # 0:2
                        lambda x: c_action * (x['case_length'] - min(x['prefix_nr'], nonmonotonic_threshold) / x[
                            'case_length']) * 3 + max((
                                                              x['prefix_nr'] - 1) / x['case_length'],
                       ...
                       [lambda x: (c_action * (x['case_length'] - min(x['prefix_nr'], nonmonotonic_threshold) / x[
                           'case_length']) / 2) + (
                                              c_com * (x['case_length'] - x['prefix_nr'] / x['case_length'])) * 1.5,
                        # 0:2
                        lambda x: (c_action * (x['case_length'] - min(x['prefix_nr'], nonmonotonic_threshold) / x[
                            'case_length']) / 2) + max((
                                                               x['prefix_nr'] - 1) / x['case_length'],
    ...
    costs1 = np.matrix([[lambda x: 0,
                        lambda x: c_miss],
                       [lambda x: c_action * (x['case_length'] - min((x['prefix_nr'] + 1), nonmonotonic_threshold) / x[
                           'case_length']) + (c_com * (x['case_length'] - (x['prefix_nr'] + 1) / x['case_length'])),
                        lambda x: c_action * (x['case_length'] - min((x['prefix_nr'] + 1), nonmonotonic_threshold) / x[
                            'case_length']) + max((
                                                          (x['prefix_nr'] + 1) - 1) / x['case_length'],
                                                  max_nonmonotonic_threshold) * c_miss
                        ],
                       [lambda x: c_action * (x['case_length'] - min((x['prefix_nr'] + 1), nonmonotonic_threshold) / x[
                           'case_length']) * 3 + (c_com * (x['case_length'] - (x['prefix_nr'] + 1) / x['case_length'])) / 2,
                        # 0:2
                        lambda x: c_action * (x['case_length'] - min((x['prefix_nr'] + 1), nonmonotonic_threshold) / x[
                            'case_length']) * 3 + max((
                                                              (x['prefix_nr'] + 1) - 1) / x['case_length'],
                       ...
                       [lambda x: (c_action * (x['case_length'] - min((x['prefix_nr'] + 1), nonmonotonic_threshold) / x[
                           'case_length']) / 2) + (
                                              c_com * (x['case_length'] - (x['prefix_nr'] + 1) / x['case_length'])) * 1.5,
                        # 0:2
                        lambda x: (c_action * (x['case_length'] - min((x['prefix_nr'] + 1), nonmonotonic_threshold) / x[
                            'case_length']) / 2) + max((
                                                               (x['prefix_nr'] + 1) - 1) / x['case_length'],
...
for c_miss_weight, c_action_weight in cost_weights:
    for c_com_weight in c_com_weights:
        for early_type in ["nonmonotonic"]:
