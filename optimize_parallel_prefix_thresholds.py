...
def evaluate_model_cost(args):
    ...
    for nr_events in range(1, dt_preds.prefix_nr.max() + 1):
        ...
        tmp = tmp[tmp.predicted_proba >= conf_threshold[nr_events-1]]
        tmp["prediction"] = 1
...
def run_experiment(c_miss_weight, c_action_weight, c_com_weight, early_type):
    ...
    conf_thresholds = []
    for i in range(1,dt_preds.prefix_nr.max() + 1):
    ...
    space = {'conf_threshold': conf_thresholds,
             ...
             'c_com': c_com}
    trials = Trials()
    best = fmin(evaluate_model_cost, space, algo=tpe.suggest, max_evals=1000, trials=trials)
