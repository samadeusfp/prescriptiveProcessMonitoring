...
def evaluate_model_cost(args):
    ...
    conf_threshold2 = args['conf_threshold2']
    ...
    if early_type == "linear":
        ...
        costs = np.matrix([[lambda x: 0,
                            lambda x: c_miss],
                           [lambda x: (c_action * (
                                   1 - min(x['prefix_nr'], nonmonotonic_threshold) / x[
                               'case_length'])) + (c_com * (
                                   1 - x['prefix_nr'] / x['case_length'])),
                            lambda x: (c_action * (
                                    1 - min(x['prefix_nr'], nonmonotonic_threshold) / x[
                                'case_length'])) + (max((x['prefix_nr'] - 1) / x['case_length'],
                                                        min(max_nonmonotonic_threshold / x['case_length'], 1)) * c_miss)
                            ]])
    else:
        costs = np.matrix([[lambda x: 0,
                            lambda x: c_miss],
...
def run_experiment(c_miss_weight, c_action_weight, c_com_weight, early_type):
    ...
    prefix_threshold = conf['prefix_threshold']
    conf_threshold = conf['conf_threshold']

    space = {'conf_threshold2': hp.uniform("conf_threshold", 0, 1),
