...
def write_test_set_predictions(preds, y_test, dt_test_prefixes, dataset_name, dataset_manager):
...
def write_error_for_prefix_length(dataset_name, min_prefix_length, max_prefix_length, dt_preds_val):
...
def write_auc(dataset_name, min_prefix_length, max_prefix_length, dt_preds):
...
def write_delta(min_prefix_length, max_prefix_length, dataset_name, dt_preds_val):
...
pWriteTestSetPredictions = Process(target=write_test_set_predictions,
                                   args=(preds, y_test, dt_test_prefixes, dataset_name, dataset_manager))
pWriteTestSetPredictions.start()

print(time.time() - start)
# write AUC for every prefix length
pWriteAuc = Process(target=write_auc, args=(dataset_name, min_prefix_length, max_prefix_length, dt_preds))
pWriteAuc.start()
...
pWriteDelta = Process(target=write_delta, args=(min_prefix_length, max_prefix_length, dataset_name, dt_preds_val))
pWriteDelta.start()
